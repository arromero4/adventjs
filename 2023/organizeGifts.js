...
// Cada 5 cajas (50) se apilan en un palé, representado por [x]. Por ejemplo, 10 cajas de a se apilan en 2 palés de esta manera: [a][a]
...
function organizeGifts(gifts) {
  ...
  numbers.forEach((item, index) => {
    let typeOfGift = letters[index];
    let quantity = parseInt(item);

    let pale = Math.floor(quantity / 50);
    let box = Math.floor((quantity % 50) / 10);
    let remainingGifts = quantity % 10;

    result += `[${typeOfGift}]`.repeat(pale);//add pale
    result += `{${typeOfGift}}`.repeat(box);//add box

    if (remainingGifts > 0) {
      result += `(${typeOfGift.repeat(remainingGifts)})`;//add remaining items
    }
